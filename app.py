...
with tabs[0]:
    ...
    seo_d = c2.text_area("Meta Description", "Stop paying monthly fees for web hosting.", height=100)
...
with tabs[5]:
    ...
    booking_embed = st.text_area("Embed Code", height=150, value='<!-- Calendly --><div class="calendly-inline-widget" data-url="https://calendly.com" style="min-width:320px;height:630px;"></div><script type="text/javascript" src="https://assets.calendly.com/assets/external/widget.js" async></script>')
...
# --- 5. COMPILER ENGINE (MINIFIED & ROBUST) ---
...
def get_theme_css():
    ...
    if card_hover_style == "Primary Color Border": h_css += " border-color: var(--p);"
    elif card_hover_style == "Accent Color Border (Red)": h_css += " border-color: var(--s);"
...
def gen_product_page():
    ...
    return f"""{ar}<section style="padding-top:120px;"><div class="container"><a href="index.html#inventory" class="btn btn-primary" style="margin-bottom:2rem;">&larr; Back</a><div id="pd">Loading...</div></div></section><script defer>async function loadP() {{ const p=new URLSearchParams(window.location.search).get('item'); try{{ const res=await fetch('{sheet_url}'); const t=await res.text(); t.split('\\n').slice(1).forEach(l=>{{ const c=parseCSVLine(l); if(c[0]===p){{ let m=`<img src="${{c[3]?c[3].split('|')[0]:''}}" style="width:100%;border-radius:12px;">`; if({str(enable_ar).lower()}&&c.length>5&&c[5].includes('.glb')) m=`<model-viewer src="${{c[5]}}" ar auto-rotate style="width:100%;height:400px;"></model-viewer>`; document.getElementById('pd').innerHTML=`<div class="detail-view"><div>${{m}}</div><div><h1 style="font-size:3rem">${{c[0]}}</h1><h2 style="color:var(--s)">${{c[1]}}</h2><p>${{c[2]}}</p><button onclick="addToCart('${{c[0]}}','${{c[1]}}')" class="btn btn-accent" style="width:100%;margin-top:2rem">Add to Cart</button></div></div>`; document.title=c[0]+" | {biz_name}"; }} }}); }}catch(e){{}} }} window.addEventListener('load',loadP);</script>"""

# --- 6. ASSEMBLY ---
home_c = gen_hero() + f'<section style="background:var(--card)"><div class="container grid-3">' + "".join([f'<div class="card reveal"><h3 style="color:var(--s)">{l.split("|")[1]}</h3><p>{l.split("|")[2]}</p></div>' for l in feat_data_input.split('\n') if '|' in l]) + '</div></section>' + gen_inventory()
